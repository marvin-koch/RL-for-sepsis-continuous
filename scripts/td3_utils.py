...
def direct_eval_rnn(rl_policy, replay_buffer,  vc_range, device, parameters, action_dim, state_dim):
    """
    eval_policy: the policy to be evaluated
    replay_buffer: test replay buffer with bootstrap == True
    iv_range and vc_range: the min and max actions for the 2 actions, used to create uniform random policy
    ...
    while eval_iters < parameters["eval_steps"]:
        ...
        ##NOT SURE WITH REWARD
        (action_rl, _, _, _), internal_state = rl_policy.act(internal_state, action, scores[:,2].unsqueeze(1), state, True)
        ...
        # We are evaluating each strategy
        # SCORE OR REWARD???
        action = torch.stack(
            (ptu.zeros((batch_size, action_dim)).float(), action), dim=0
        )
        action_c = torch.stack(
            (ptu.zeros((batch_size, action_dim)).float(), action_c), dim=0
        )
        pre_action_rl = action_rl
        action_rl = torch.stack(
            (ptu.zeros((batch_size, action_dim)).float(), action_rl), dim=0
        )
        action_zero = torch.stack(
            (ptu.zeros((batch_size, action_dim)).float(), action_zero), dim=0
        )
        action_rd = torch.stack(
            (ptu.zeros((batch_size, action_dim)).float(), action_rd), dim=0
        )
        reward = torch.stack(
        ...
        Q1_estimate, Q2_estimate = rl_policy.critic_target(action, next_scores, observs, action_rl)
        Q_estimate = torch.min(Q1_estimate, Q2_estimate).cpu()
        

        Q1_clinician, Q2_clinician = rl_policy.critic_target(action, next_scores, observs, action_c)
        Q_clinician = torch.min(Q1_clinician, Q2_clinician).cpu()


        Q1_random, Q2_random = rl_policy.critic_target(action, next_scores, observs, action_rd)
        Q_random = torch.min(Q1_random, Q2_random).cpu()

        Q1_zero, Q2_zero = rl_policy.critic_target(action, next_scores, observs, action_zero)        
        Q_zero = torch.min(Q1_zero, Q2_zero).cpu()
...
def rnn_plot_action_sofa(rl_policy, replay_buffer,  parameters, action_dim):
    """
    """
    eval_iters = 0
    ivs_rl = []
    ...
    while eval_iters < parameters["eval_steps"]:
        ...
        action_rl =  rl_policy.actor(action, scores, observs)
...
def rnn_plot_action_dist(rl_policy, replay_buffer,  parameters, action_dim):
    f, (ax1, ax2) = plt.subplots(2, 1, sharey=False)
    ax1 = plt.subplot(2, 1, 1)
    ...
    while eval_iters < parameters["eval_steps"]:
        ...
        action_rl =  rl_policy.actor_target(action, next_scores, observs)
        action_rl = action_rl[0]

        # print("shape", action_rl[0].shape)
...
def rnn_u_plot_vc(rl_policy, replay_buffer, device, parameters, action_dim):
    ...
    while eval_iters < parameters["eval_steps"]:
        ...
        observs = torch.stack((state, next_state), dim=0)  # (T+1, B, dim)
        next_action_c = torch.stack(
            (ptu.zeros((batch_size, action_dim)).float(), next_action_c), dim=0
        )
        scores = torch.stack(
            (ptu.zeros((batch_size, 1)).float(), (scores[:,2]).reshape(-1,1)), dim=0
        ) 
...
def rnn_u_plot_iv(rl_policy, replay_buffer, device, parameters, action_dim):
    ...
    while eval_iters < parameters["eval_steps"]:
        ...
        batch_size = action_c.shape[0]
        print(action_rl.shape, next_action_c.shape)
        iv_r = action_rl[:,0] - next_action_c[:,0]
        iv_n = no_action[:, 0] - next_action_c[:, 0]
